...
def tazi(number):
    try:
        ...
        r = requests.post(url=url, headers=headers, json=payload, timeout=10)
        if r.status_code == 200:
            return True, "Tazı"
        else:
            return False, "Tazı"
...
def joker(number):
    try:
        ...
        r = requests.post(url=url, headers=headers, data=payload, timeout=10)
        r1 = json.loads(r.text)["success"]
...
def send_service(number, service):
    ...
    try:
        ...
        if result[0]:
            all_sends += 1
            success_sends += 1
            print(f"[+] {all_sends} {result[1]} Başarılı!")
        else:
            all_sends += 1
            failed_sends += 1
            print(f"[-] {all_sends} {result[1]} Başarısız.")
    except Exception as e:
        all_sends += 1
        failed_sends += 1
        print(f"[-] {all_sends} {service.__name__} Hata: {str(e)}")
...
def watermark():
    print(""" 
  ____  _____ ___  _   _ _        _    __  __
 |  _ \| ____/ _ \| \ | | |      / \   \ \/ /
 | |_) |  _|| | | |  \| | |     / _ \   \  / 
 |  _ <| |__| |_| | |\  | |___ / ___ \  /  \ 
 |_| \_\_____\___/|_| \_|_____/_/   \_\/_/\_\\
""")
...
def get_number():
    ...
    while True:
        try:
            number = int(input(f"""Telefon numarasını yazın. "54XXX" \n[?] : """))
...
def get_amount():
    ...
    while True:
        try:
            amount = int(input(f"""Kaç SMS gönderilsin? "0" sonsuz gönderir basın.\n[?] : """))
            if amount >= 0:
                return amount
            else:
                clear()
...
def restart():
    global clear
    while True:
        question = input(f"Devam edilsin mi?\n[Y/N] : ").upper().replace(" ", "")
        if question == "N":
            quit()
        elif question == "Y":
            clear()
